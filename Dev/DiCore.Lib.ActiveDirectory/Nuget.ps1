...
If (Test-Path -Path $SettingsFile)
{
    ...
    If ($setNode -ne $null)
    {
        If($setNode.InnerText.ToUpper().StartsWith("t".ToUpper()))
        {
            ...
            $fileNode = $nuspecXml.CreateElement('file')
            $fileNode.SetAttribute('src','bin\release\*.dll')
    ...
    If ($setNode -ne $null)
    {
        If($setNode.InnerText.ToUpper().StartsWith("t".ToUpper()))
        {
            ...
            $fileNode = $nuspecXml.CreateElement('file')
            $fileNode.SetAttribute('src','bin\release\*.pdb')
            $fileNode.SetAttribute('target','lib')
            $filesNode.AppendChild($fileNode)
        }
    } 
...
Get-ChildItem -Path $SearchPath -Include "*.nupkg" -Recurse | ForEach-Object {& $nugetPath @('push') @($_.FullName) @('d7c0933a-0606-46f9-bb69-53e65d2fbd0f') @('-s') @('http://nuget.ctd.tn.corp/') @('-Verbosity') @('detailed')}
