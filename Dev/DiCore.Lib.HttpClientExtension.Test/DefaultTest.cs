...
{
    public class DefaultTest : IClassFixture<DefaultTestFixture>
    {
        //private readonly IRestClient client;
        private readonly IStorage storage;
        private readonly HttpClient client;
        private readonly HttpClient personsClient;

        private const string PersonsRoute = "persons";
        ...
        public DefaultTest(DefaultTestFixture defaultTestFixture)
        {
            storage = defaultTestFixture.ServiceProvider.GetRequiredService<IStorage>();

            //var clientFactory = defaultTestFixture.ServiceProvider.GetRequiredService<IRestClientFactory>();
            //client = clientFactory.GetRestClient("default");
            var factory = defaultTestFixture.ServiceProvider.GetRequiredService<IHttpClientFactory>();
            client = factory.CreateClient("main");
            personsClient = factory.CreateClient("persons");
        ...
        [Fact]
        public async void GetEmptyUrlManyAsync()
        {
            var storageItems = storage.GetPersons().ToArray();
            var apiItems = await personsClient.GetAsync<PersonItem[]>(string.Empty);
            Assert.True(apiItems.Data.ItemEquals(storageItems), "Get many async");
        }

        [Fact]
        public async Task GetEmptyUrlOneAsync()
        {
            var storageItem = storage.GetPersons().First();
            var apiItem = await personsClient.GetAsync<PersonItem>(String.Empty, storageItem.Id);
            Assert.True(storageItem.Equals(apiItem.Data), "Get one async");
        }
    }
}